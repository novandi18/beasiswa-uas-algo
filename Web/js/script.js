...
form.addEventListener("submit", function getData() {
    ...
    let nimVal = document.getElementById('nim').value;
    let ipkVal = document.getElementById('ipk').value;
    let gajiVal = document.getElementById('gaji').value;

    ipkfloat = parseFloat(ipkVal);
    // Hilangkan tanda titik pada string gaji
    let gajiDot = gajiVal.split('.').join("");
    let gajiInt = parseInt(gajiDot);

    if(ipkfloat > 3 && gajiInt <= 2000000) {
        // Tambahkan gaji tambahan dengan tunjangan untuk keluarganya
        statusFix = "Penerima";
    } else {
        // Gaji tidak berubah
        statusFix = "Bukan Penerima";
    }
    ...
    let keynim = data[endKey].nim;
    let keyipk = data[endKey].ipk;
    ...
    document.getElementById('nimHasil').innerHTML = keynim;
    document.getElementById('ipkHasil').innerHTML = keyipk;
...
function tableData() {
    ...
    let headers = ['Nama', 'NIM', 'IPK', 'Gaji Ortu', 'Status'];
